...
The game is designed from 390 CSS pixels upward, uses safe-area padding, and caps its phone surface near 460 pixels on desktop. When no valid local `PlayerProfile` exists, the entry route is a one-column identity setup page with a compact first-election premise, live member preview, a six-choice avatar filmstrip, editable name, adjacent random-name action, bounded authored team select, and one primary “进入成员主页” commit. The filmstrip leads with the kitten, puppy, and plush-toy choices, followed by three original everyday idol portraits; the leading kitten is also the default. Its circular images have accessible radio names but no visible caption or explanatory row. Narrow screens retain horizontal scrolling, while desktop-width viewports fit all six choices in one row without horizontal overflow. It stays a native radio group and never becomes a separate onboarding step. The premise derives its day count from the active story pack, identifies the player only as a new fictional idol without stating an age, and replaces mechanical help copy rather than adding a second onboarding panel. Later visits go directly to the member profile header, account stats, collection row, two-column workbench, and game-viewport-fixed pocket-style bottom tab bar. Its player-facing surfaces stay in one light theme; the dark production-desk theme belongs only to the separately built editor. The bottom tab bar uses the main account surface, while the workbench surface grows behind it to absorb any remaining viewport height without moving the workbench content. “翻牌” enters the reply inbox immediately, starting or resuming locally as needed; identity editing, display mode, and restart live in the member settings sheet. Restart uses one hold-to-confirm danger control whose background fills over 1.5 seconds; a tap, early release, pointer exit, cancellation, or lost focus leaves the run untouched. Restarting a run never removes the member identity. The three unavailable workbench cards remain visibly defocused and non-interactive.
...
Each row opens a light, chronological chat transcript containing the reached history for that contact. The sticky contact title repeats the same ordered trait tags shown in the list, while the message bubbles remain uncluttered. A pending core row initially keeps its choices collapsed behind one viewport-bottom composer-shaped button labeled “点此选择回复…”, with a familiar right-side “发送” action and send icon, protecting the transcript as the first reading state; activating that native button reveals and focuses the complete preset choice section without opening a keyboard or creating a free-text field. Another pending item from the same fan remains a separate list entry and choice flow. Every exchange receives its own centered day marker. Fan messages sit in left-aligned white bubbles, completed idol replies sit in right-aligned lavender bubbles, and expired entries keep their original incoming body plus an explicit expired status. Core reply bubbles retain resource costs. After a reply, the result ticket records the current fan's affinity direction through a Lucide icon, text, and semantic color: standard mode shows the signed number, while realistic mode shows up, down, or unchanged. It does not add authored judgment copy. A read-only ordinary-NPC round may contain one main incoming bubble plus up to four consecutive incoming bubbles from the same contact; it never fabricates a player reply, shows candidate cards, or opens a result screen.

Preset long replies remain ordinary choices rather than a separate mode. The mobile choice card, immediate sent-message result, and later idol chat bubble show the complete text with authored paragraph breaks preserved. They do not line-clamp, open a confirmation step, or create an internal card scrollbar; the active game shell remains the only scroll owner and the cost row follows the text. The editor gives a non-blocking reminder after 140 characters, but no hard content limit or extra gameplay field is introduced.

The flip workflow names one game scroll owner per layout mode. Below 680px, the browser document keeps native mobile scrolling; at 680px and above, the centered phone frame is one bounded, rounded scroll viewport while the outer desktop page stays fixed. Its natural-flow content reserves bottom space equal to the fixed action rail's full outer height, including the device safe-area inset, so the last row and focused controls remain reachable above it. The rail is fixed flush to the active game viewport's bottom edge—browser viewport on mobile, simulated phone edge on desktop—even when the conversation feed grows beyond one screen. Its fully opaque surface paints continuously through the safe area and prevents scrolled list content from showing beneath or through it. The rail remains centered and width-capped with the phone frame, keeps “点外卖” beside “几天后”, and does not show the run's accumulated takeout count; on the final turn, “几天后” becomes “进入总选”. Ordering takeout opens one compact, receipt-like feedback dialog with a randomly selected familiar shop name, a locally bundled photograph that visibly uses takeaway packaging, and the configured fixed “精力 +X / 心情 +X” recovery shown in the action tip. Soup-like orders use a single-serving takeaway bowl rather than a tabletop hotpot scene. It confirms the completed utility action rather than adding another choice layer. After the third and fourth receipts close, content-defined warning copy briefly floats at the visual center above the workbench without taking focus or blocking pointer input; the fifth order enters the takeout ending. The reminder uses a dark translucent capsule so the authored text stays readable over the feed, and reduced-motion mode keeps only a short fade.
...
The settings restart control is the game-side destructive exception: it uses a low-emphasis danger surface at rest and turns the button background into a determinate progress fill while held. Its label changes to “继续按住…” without changing geometry. Pointer and keyboard holds are equivalent, and reduced-motion mode removes the rotating icon while preserving the functional progress indication.
