...
```json
{
  ...
  "tag": "冒泡",
  "message": "我去，公告看了三遍，计算器先申请退河了。",
  "continuations": ["这到底是作品赛，还是数学竞赛披了件 MV 外套？", "已老实，先等成片。"]
}
```
...
- `reply` 可选：填写时保留原有自动一问一答，在聊天中显示 NPC 左侧消息和成员右侧回复。
- 不填写 `reply` 时，该轮是玩家只读的 NPC 闲聊。`continuations` 可选，包含 1–4 条同一 NPC 紧接着发出的左侧气泡；只有第一条 `message` 也合法。
